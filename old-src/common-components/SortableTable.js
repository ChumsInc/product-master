        ...
        return (
            ...
            <tr>
                {fields.map(({field, title = null, noSort = false, className = ''}) => (
                    <ThSortable key={field} field={field}
                                currentSort={sort} onClick={this.props.onClickSort}
                                ...
                                <tr key={key} onClick={() => this.onSelectRow(row)}
                                    ...
                                    {fields.map((field, index) => {
                                        if (typeof field.render === 'function') {
                                            return (
                                                <td key={index} className={classNames(field.className)}>{field.render(row)}</td>);
                                        }
                                        return (<td key={index} className={classNames(field.className)}>{row[field.field] === null ?
